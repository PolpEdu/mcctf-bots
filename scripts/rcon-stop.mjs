...
const command = process.argv[5] ?? 'stop'
...
sock.on('connect', () => {
  sock.write(pack(1, TYPE_AUTH, password))
})
...
sock.on('error', (err) => { console.error('rcon error:', err.message); process.exit(1) })
sock.on('close', () => { console.log(`rcon: sent "${command}"`); process.exit(0) })
